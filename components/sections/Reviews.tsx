...
export default function ReviewsSection() {
  ...
  const indexRef = useRef(N); // start at the second copy
  const slidingRef = useRef(false);
  const pausedRef = useRef(false);
  const intervalRef = useRef<ReturnType<typeof setInterval> | null>(null);

  // Calculate offset so the active card is centered in the wrapper
  const getOffset = (idx: number) => {
    const wrapper = wrapperRef.current;
    if (!wrapper) return 0;
    const wrapperWidth = wrapper.offsetWidth;
    return -(idx * STEP) + wrapperWidth / 2 - CARD_WIDTH / 2;
  };
  ...
  useEffect(() => {
    // Wait one frame so wrapper has measured width
  ...
  return (
      <section className="py-24 relative">
        ...
        {/* Carousel wrapper — no overflow-hidden here so cards aren't clipped */}
        <div
            ref={wrapperRef}
            className="relative w-full py-6 mb-10"
            onMouseEnter={() => { pausedRef.current = true; }}
            onMouseLeave={() => { pausedRef.current = false; }}
        >
          {/* Edge fade — left */}
          <div
              className="absolute top-0 bottom-0 left-0 w-32 md:w-56 z-10 pointer-events-none"
              style={{ background: "linear-gradient(to right, #0f172a 30%, transparent)" }}
          />
          {/* Edge fade — right */}
          <div
              className="absolute top-0 bottom-0 right-0 w-32 md:w-56 z-10 pointer-events-none"
          ...
          >
            {ITEMS.map((review, i) => (
                ...
                >
                  ...
                  {/* Quote */}
                  <p className="text-sm leading-7 text-slate-300/80 italic mb-5 flex-1">
                    "{review.comment}"
                  </p>
        ...
        <div className="flex items-center justify-center gap-4 px-6">
          <button
              aria-label="Previous"
              onClick={() => slide(-1)}
              className="w-10 h-10 rounded-full border border-white/10 bg-white/5 text-slate-300 flex items-center justify-center transition-all duration-200 hover:bg-indigo-500/20 hover:border-indigo-500/50"
          >
